...
import {
  addAppointmentAC,
  editAppointmentAC,
  removeAppointmentByIdAC,
  searchAppointmentAC,
} from "./state/appointment-reducer";
...
import {
  ...
  removeDoctorByIdAC,
  searchDoctorAC,
} from "./state/doctor-reducer";
import { DoctorType } from "./state/doctor.type";
import { ProfessionType } from "./state/profession.type";
import { AppRootState } from "./state/store";

const App = () => {
  const dispatch = useDispatch();
  const clients = useSelector<AppRootState, Array<ClientType>>(
    (state) => state.clients,
  );
  const doctors = useSelector<AppRootState, Array<DoctorType>>(
    (state) => state.doctors,
  );
  const diseases = useSelector<AppRootState, Array<DiseaseType>>(
    (state) => state.diseases,
  );
  const diagnosis = useSelector<AppRootState, Array<DiagnosType>>(
    (state) => state.diagnosis,
  );
  const appointments = useSelector<AppRootState, Array<AppointmentType>>(
    (state) => state.appointments,
  );
  const professions = useSelector<AppRootState, Array<ProfessionType>>(
    (state) => state.professions,
  );
  const categoryDiseases = useSelector<
    AppRootState,
    Array<CategoryDiseasesType>
  >((state) => state.categoyDiseases);
        ...
        <Routes>
          ...
          <Route
            ...
            element={
              <ListDiseases
                ...
                handleSearchDisease={handleSearchDisease}
          ...
          <Route
            ...
            element={
              <ListDiagnosis
                diagnosis={diagnosis}
                handleAddDiagnos={handleAddDiagnos}
                handleDeleteDiagnos={handleDeleteDiagnos}
                handleEditDiagnos={handleEditDiagnos}
                handleSearchDiagnos={handleSearchDiagnos}
                doctors={doctors}
                clients={clients}
                diseases={diseases}
              />
